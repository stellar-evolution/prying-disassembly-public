...
with serial.Serial('COM7', 9600) as ser:
    ...
    while rval:
        ...
        # In the while loop, after obtaining FSR_readings and xy_coordinates, append them to the lists
        xy_coordinates = [x_coords[0], y_coords[0], x_coords[1], y_coords[1], x_coords[2], y_coords[2], x_coords[3], y_coords[3]]
        # for ij in range(len(xy_coordinates)):
        #     xy_coordinates_list.append(xy_coordinates[ij])
        FSR_readings_list.append(FSR_readings)
        xy_coordinates_list.append(xy_coordinates)
...
# After the while loop, convert the lists to NumPy arrays and reshape them
FSR_readings_array = np.array(FSR_readings_list)
xy_coordinates_array = np.array(xy_coordinates_list)

model = load('trained_model.joblib')

# Now, you can use these arrays with the train_test_split function
X_train, X_test, y_train, y_test = train_test_split(FSR_readings_array, xy_coordinates_array, test_size=0.2,
                                                    random_state=42)
